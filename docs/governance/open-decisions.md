...
| HD-W1-Q1 | Auth accessor standard (Wave 1) | Completion Wave 1 | Helper-only `auth('identity')` vs allowlist | Lead | Wave 1 | **RESOLVED — B** | ONLY `auth('identity')` globally; **KEEP** Auth Foundation `SessionAuthenticator` / `SessionAuthUserResolver` on default/web + allowlist (`docs/audit/auth-accessor-allowlist.md`). Resolver not moved to identity (would break LoginUserAction pairing). |
| HD-W1-Q2 | Integer PK exceptions | Completion Wave 1 | Migrate vs KEEP package/framework | Lead | Wave 1 | **RESOLVED — KEEP** | Spatie + jobs/activity/telescope/cache/sessions/migrations KEEP integer PKs. Laravel `users` **DO NOT TOUCH** (separate HD). |
| HD-W1-Q3 | UUID v7 migration timing | Completion Wave 1 | Wave 1 migrate vs document-only | Lead | Wave 1 | **RESOLVED — DEFERRED** | Wave 1 = inventory only (`docs/audit/uuid-inventory.md`). No v7 migration code. Domain `HasUuid` already v7. |
| DEBT-W1-01 | Dual-guard `Auth::guard(api\|identity)` | Wave 1 discovery / DBT-3 | Remediate vs park | Lead | Pre-Wave 2 handoff | **OPEN — REGISTERED** | EstablishApiSession + Auth session controllers. Fix = STOP-F without UNFREEZE DBT-3. See `docs/audit/wave1-debt-discovery-01.md`. |
| DEBT-W1-02 | `auth('api')` EmployeeRecordController | Wave 1 discovery / DBT-3 | Remediate vs park | Lead | Pre-Wave 2 handoff | **OPEN — REGISTERED** | Same file cluster as DBT-3. |
| DEBT-W1-07 | `$request->user()` middleware | Wave 1 discovery | Remediate vs park | Lead | Pre-Wave 2 handoff | **OPEN — REGISTERED** | Audit + Request mutation middlewares; not flagged by T3 rule yet. |
| W2-SOFT-COUPLING-01 | Dormitory Livewire `#[Layout('components.layouts.dashboard')]` | Completion Wave 2 Isolation | Accept vs rename vs Shared extract | Lead | STOP-2 | **ACCEPTED** | Option A: PHP isolation sufficient; layout string not a PHP dependency. Evidence: `docs/audit/wave2-debt-discovery.md`. Wave 2 **COMPLETE**. |
| DEBT-W3-01 | CheckIn does not advance Request `checked_in`/`checked_out` | Completion Wave 3 W3-B | Wire CheckIn→Request vs park | Lead | Post W3-B | **CLOSED** | Wired via `RequestStayLifecycleCommandPort` + `RequestStayLifecycleCommandBridge`. Evidence: CheckIn/CheckOut actions + `CheckInOutFlowTest`. |
| W3-WP-WF-04-RISK | Baseline Request transition / WP-WF-04 cutover failures | Wave 3 / WF cutover | Treat as known-risk vs block W3 | Lead | W3-B | **KNOWN-RISK** | Not a W3-B blocker. `docs/audit/wave3-wp-wf-04-known-risk.md`. |
...
| تاریخ | تغییر | توسط |
|-------|-------|------|
| 2026-07-21 | **DEBT-W3-01 CLOSED:** CheckIn/CheckOut Actions call `RequestStayLifecycleCommandPort`; bridge advances Request `checked_in`/`checked_out` for request-sourced allocations (no-op without source_request_id). HD-02/HD-03/DBT-3 untouched. | Agent (Lead DEBT-W3-01 resolution) |
| 2026-07-21 | **Completion Wave 3 W3-B COMPLETE:** STOP-3A/B approved. OA-05-03 Spatie states on Request (`waiting_for_allocation`→`allocated`→`checked_in`→`checked_out` + `allocation_failed`). `RequestLifecycleCommandAdapter` wired. DEBT-W3-01 OPEN (CheckIn consumer). WP-WF-04 baseline = KNOWN-RISK. Artifacts under `docs/audit/wave3-*`. HD-02/HD-03/DBT-3 untouched. | Agent (Lead APPROVE W3-B) |
| 2026-07-21 | **Completion Wave 2 COMPLETE (Isolation):** STOP-2 Option A approved. PHP Dormitory↛Dashboard imports = 0; Entrypoint/Provider PASS. Soft layout coupling registered as **W2-SOFT-COUPLING-01 ACCEPTED**. Artifacts: `docs/audit/wave2-isolation-discovery.md`, `wave2-debt-discovery.md`. No Livewire/layout/code changes. Frozen scope untouched. | Agent (Lead APPROVE STOP-2 Option A) |
| 2026-07-21 | **HANDOFF Wave 1 COMPLETE → pre-Wave 2:** T1–T4 done. DEBT-DISCOVERY-01 audited: DEBT-W1-01/02/07 OPEN (DBT-3 / request.user); DEBT-W1-03 ACCEPTED allowlist; DEBT-W1-04 tooling; DEBT-W1-05 known-fail suite; DEBT-W1-06 users PK pending. Artifacts: `docs/audit/completion-wave1-handoff.md`, `wave1-debt-discovery-01.md`. Wave 2 **SUSPENDED** until Lead scope + `BEGIN WAVE 2`. | Agent (BEGIN HANDOFF) |
| 2026-07-21 | **HD-W1-Q1/Q2/Q3 RESOLVED (Wave 1):** Q1=B Auth Foundation KEEP+allowlist (`SessionAuthenticator`/`SessionAuthUserResolver`); Q2=KEEP Spatie/framework integer PKs (`users` untouched); Q3=UUID inventory only (`docs/audit/uuid-inventory.md`). T2/T3 PARKED. Baseline 43 fail known-out-of-scope (`docs/audit/wave1-baseline-known-fail.md`). | Agent (Lead Wave 1 disposition) |
