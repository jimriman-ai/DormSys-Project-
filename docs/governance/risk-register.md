...
| BL-B1-01 | **B1 deferred — schema gap if dependent feature ships** (historical: tables removed from Phase G tip) | High (assignment/occupancy UI cannot ship) | High if Stage-3 / assignment UI started without tables | Tech Lead | Schema restored RM-BL-B1 (2026-07-16): `dormitory_manager_assignments` + `dormitory_unit_manager_assignments` (`2026_07_16_000001/000002`); dashboards wired; B1 tests restored. Commit **`369a106`** on `release/f2-employee-auth-ui-l9`. | On Phase H start or consumer needing assignments | **RESOLVED** (on branch; program merge SHA pending Lead — GAP-GOV-02) |
...
Status: **restored** under RM-BL-B1 — commit `369a106` on branch. Prior “deleted” list retained for audit trail.
