...
این بخش را می‌توانی داخل README اصلی پروژه، مثلاً نزدیک بخش Architecture / Development Rules قرار بدهی.

```md
## Architecture Governance

DormSys is a backend-authoritative enterprise workflow system with a thin but interaction-capable UI.

The project is governed by explicit architecture contracts to prevent logic leakage, preserve workflow clarity, and keep business meaning authoritative in backend layers.
...
Where action availability matters, the backend should provide explicit capability fields such as:
...
If implementation convenience conflicts with a governing architecture contract, the contract wins.
