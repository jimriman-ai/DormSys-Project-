...
| **DP-ALLOC-ITEM-BED-FK** | Should `allocation_items.bed_id` gain a physical FK to `dormitory_beds.id`, remain soft UUID (AP-04), or drop Eloquent `bed()`? | `.dormSys/database-map.md` §allocation_items `bed_id` Notes `—`; mig `…/allocation/2026_07_01_000002_…` FK only on `allocation_id`; `AllocationItemModel.php` Eloquent `bed()` + Eloquent-only PHPDoc; `.dormSys/decision-package-01.md` §3 | LEAD | **OPEN** — Packaged DECISION-PACKAGE-01, 1405/05/01 / 2026-07-23. |
| **DP-BED-SIGNAL-OWNERSHIP** | Should Domain `Bed` own `last_signal_reference_id`, or remain Infra/App-only (Persistence `BedModel` + Allocation physical-state services)? | Map §dormitory_beds `last_signal_reference_id`; `BedModel.php` fillable; `Domain/Entities/Bed.php` no signal property; `AllocationBedPhysicalStateRepository.php`; `.dormSys/decision-package-01.md` §4 | LEAD | **OPEN** — Packaged DECISION-PACKAGE-01, 1405/05/01 / 2026-07-23. |
| **DP-XMOD-BELONGS** | Ratify Persistence cross-module Eloquent `belongsTo` (AP-04 soft UUID) as exception to `AGENTS.md`, unwind to ports-only, or allowlist hybrid? | `AGENTS.md` Architecture Rules §2; e.g. `RequestModel` employee/dormitory, `AllocationItemModel::bed()`, Voucher/Lottery cross-module belongsTo; `.dormSys/decision-package-01.md` §5 | LEAD | **OPEN** — Packaged DECISION-PACKAGE-01, 1405/05/01 / 2026-07-23. |
